...
function taillog {
    if [ -f /var/solr/logs/solr-8983-console.log ]; then
        rm /var/solr/logs/solr-8983-console.log
    fi
    tail -F /var/solr/logs/solr-8983-console.log &
}
...
ZK_SERVICE=${ZK_SERVICE:-"solr-zk/zk"}
...
if [ "$CONFD_INTERVAL" -gt "0" ]; then
    ...
    echo "/usr/bin/nohup /usr/bin/confd ${CONFD_PARAMS} > /var/solr/logs/confd.log 2>&1 &" >> ${CONFD_SCRIPT}
